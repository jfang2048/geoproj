"""In-memory SCS-CN sensitivity preview for generated response units."""
...
from postfire_runoff.frontend.components.loaders import RAINFALL_EVENTS, RUNOFF_UNITS, load_csv
...
def preview_metrics(
    ...
    if units is None or rain is None:
        ...
        return result
    required = {"baseline_cn", "burn_class", "area_m2"}
    if not required.issubset(units.columns):
        result["note"] = f"Runoff units missing required columns. Need: {sorted(required)}"
        return result
    ...
    try:
        baseline_cns = units["baseline_cn"].astype(float).to_numpy()
    ...
    result.update({
        ...
        "note": "Preview only — official outputs are unchanged. Footprint scenarios require separate burn masks and reruns.",
    })
    return result
...
def preview_curve(
    ...
    units = load_csv(RUNOFF_UNITS)
    if units is None or "baseline_cn" not in units.columns:
        return None
    adjustments = {0: 0.0, 1: float(cn_adj_low), 2: float(cn_adj_moderate), 3: float(cn_adj_high)}
    base_cn = float(units["baseline_cn"].median())
