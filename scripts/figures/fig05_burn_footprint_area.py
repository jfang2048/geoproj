"""Atomic figure: burned-footprint scenario area hierarchy. One PNG output."""
...
OUT = LATEX / "fig05_burn_footprint_area.png"
SCENARIO_LABELS = {
...
def main() -> None:
    ...
    values = data["burned_area_ha"].to_numpy(float)

    apply_style()
    fig, ax = plt.subplots(figsize=(5.8, 3.3))
    bars = ax.barh(labels, values, color=colors, edgecolor="black", linewidth=0.25)
    for bar, value in zip(bars, values):
        ax.text(value + 4, bar.get_y() + bar.get_height() / 2, f"{value:.1f} ha", va="center", fontsize=7, color=C_DARK)
    ax.set_xlabel("Burned area inside model domain (ha)")
